...
class HeaderMenu extends Component {
  ...
  handleJoinClicked = () => {
    ...
    this.contractManager.joinContract(publicKeyBuffer, (resultEvent) => {
      ...
      if (
        resultEvent == Constant.EVENT.ON_REJECTED ||
        resultEvent == Constant.EVENT.ON_ERROR
      ) {
    ...
    if (this.account) {
      if (this.state.isLoading == false) {
        if (this.state.address) {
          var addressExplorerUrl =
            Config.ENV.ExplorerUrl + "address/" + this.state.address;
          var dropdownTrigger;

          if (this.state.avatarUrl) {
            dropdownTrigger = (
              <span>
                <Image src={this.state.avatarUrl} avatar />
                {this.state.name
                  ? this.state.name
                  : this.state.address.substr(0, 10)}
              </span>
            );
          } else {
            dropdownTrigger = (
            ...
            <Menu.Menu position="right">
              <Menu.Item>
                <Dropdown item text={Config.ENV.NetworkName}>
                  <Dropdown.Menu>{networkItems}</Dropdown.Menu>
                </Dropdown>
              </Menu.Item>
              <Menu.Item>
                <List>
              ...
              <Menu.Item>{memberInfo}</Menu.Item>
            </Menu.Menu>
          );
        } else {
          accountInfo = (
    ...
    return (
      <Menu fixed="top" color="teal" inverted>
        ...
        <Container>
          <Menu.Item>
            <a href="/">
              <Image src="static/images/logo_small.png" height={44} />
            </a>
          </Menu.Item>
